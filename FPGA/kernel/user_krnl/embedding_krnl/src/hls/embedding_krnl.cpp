...
//                             {
//                                 #pragma HLS PIPELINE II=1
//                                 ap_uint<128> feature = s_feature_in.read();
//                                 for (int i = 0; i < (128/64); i++) 
//                                 {
//                                    #pragma HLS UNROLL
//                                    currWord.data(k*128+i*64+63, k*128+i*64) = feature(i*64+63, i*64);
//                                    currWord.keep(k*16+i*8+7, k*16+i*8) = 0xff;
//                                 }
//                             }
...
#pragma HLS stream variable=s_network depth=fifo_batch_size 
...
#pragma HLS resource variable=table_PLRAM16 core=RAM_2P_URAM
    ...
    load_access_idx(s_idx_buffer_HBM0);
    load_access_idx(s_idx_buffer_HBM1);
    load_access_idx(s_idx_buffer_HBM2);
    load_access_idx(s_idx_buffer_HBM3);
    load_access_idx(s_idx_buffer_HBM4);
    load_access_idx(s_idx_buffer_HBM5);
    load_access_idx(s_idx_buffer_HBM6);
    load_access_idx(s_idx_buffer_HBM7);
    load_access_idx(s_idx_buffer_HBM8);
    load_access_idx(s_idx_buffer_HBM9);
    load_access_idx(s_idx_buffer_HBM10);
    load_access_idx(s_idx_buffer_HBM11);
    load_access_idx(s_idx_buffer_HBM12);
    load_access_idx(s_idx_buffer_HBM13);
    load_access_idx(s_idx_buffer_HBM14);
    load_access_idx(s_idx_buffer_HBM15);
    load_access_idx(s_idx_buffer_HBM16);
    load_access_idx(s_idx_buffer_HBM17);
    load_access_idx(s_idx_buffer_HBM18);
    load_access_idx(s_idx_buffer_HBM19);
    load_access_idx(s_idx_buffer_HBM20);
    load_access_idx(s_idx_buffer_HBM21);
    load_access_idx(s_idx_buffer_HBM22);
    load_access_idx(s_idx_buffer_HBM23);
    load_access_idx(s_idx_buffer_HBM24);
    load_access_idx(s_idx_buffer_HBM25);
    load_access_idx(s_idx_buffer_HBM26);
    load_access_idx(s_idx_buffer_HBM27);

    load_access_idx(s_idx_buffer_DDR0);
    load_access_idx(s_idx_buffer_DDR1);

    load_access_idx(s_idx_buffer_PLRAM0);
    load_access_idx(s_idx_buffer_PLRAM1);
    load_access_idx(s_idx_buffer_PLRAM2);
    load_access_idx(s_idx_buffer_PLRAM3);
    load_access_idx(s_idx_buffer_PLRAM4);
    load_access_idx(s_idx_buffer_PLRAM5);
    load_access_idx(s_idx_buffer_PLRAM6);
    load_access_idx(s_idx_buffer_PLRAM7);
    load_access_idx(s_idx_buffer_PLRAM8);
    load_access_idx(s_idx_buffer_PLRAM9);
    load_access_idx(s_idx_buffer_PLRAM10);
    load_access_idx(s_idx_buffer_PLRAM11);
    load_access_idx(s_idx_buffer_PLRAM12);
    load_access_idx(s_idx_buffer_PLRAM13);
    load_access_idx(s_idx_buffer_PLRAM14);
    load_access_idx(s_idx_buffer_PLRAM15);
    load_access_idx(s_idx_buffer_PLRAM16);
...
//////////////////////////////     Load Vectors & Concatenate     ////////////////////////////// 
    ...
    sendData(m_axis_tcp_tx_meta, 
             ...
             sessionID,
             useConn,
             BATCH_NUM * BATCH_SIZE * INPUT_SIZE_AXI_512 * 64, 
             pkgWordCount
             );
...
void init_plram_t_1_table(
    ...
    for (int i = 0 ; i < table_entry_num_0 / 2; i++) {
        ...
        for (int j = 0; j < axi_padded_size_0; j++) {
            table_PLRAM[start_addr_0 + (2 * i + 1) * axi_padded_size_0 + j].range(31, 0) = m_one;
            table_PLRAM[start_addr_0 + (2 * i + 1) * axi_padded_size_0 + j].range(63, 32) = m_one;
            table_PLRAM[start_addr_0 + (2 * i + 1) * axi_padded_size_0 + j].range(95, 64) = m_one;
            table_PLRAM[start_addr_0 + (2 * i + 1) * axi_padded_size_0 + j].range(127, 96) = m_one;
        }
    }
}
...
void init_plram_t_2_tables(
    ...
    for (int i = 0 ; i < table_entry_num_1 / 2; i++) {
        ...
        for (int j = 0; j < axi_padded_size_1; j++) {
            table_PLRAM[start_addr_1 + (2 * i + 1) * axi_padded_size_1 + j].range(31, 0) = m_one;
            table_PLRAM[start_addr_1 + (2 * i + 1) * axi_padded_size_1 + j].range(63, 32) = m_one;
            table_PLRAM[start_addr_1 + (2 * i + 1) * axi_padded_size_1 + j].range(95, 64) = m_one;
            table_PLRAM[start_addr_1 + (2 * i + 1) * axi_padded_size_1 + j].range(127, 96) = m_one;
        }
    }
}
...
#pragma HLS array_partition variable=s_embedding_gather_level_A dim=1 complete
    ...
    group_4_embeddings(
        s_embedding_buffer_PLRAM0, s_embedding_buffer_PLRAM1,
        s_embedding_buffer_PLRAM2, s_embedding_buffer_PLRAM3,
        s_embedding_gather_level_A[0]); 
    group_4_embeddings(
        s_embedding_buffer_PLRAM4, s_embedding_buffer_PLRAM5,
        s_embedding_buffer_PLRAM6, s_embedding_buffer_PLRAM7,
        s_embedding_gather_level_A[1]); 
    group_4_embeddings(
        s_embedding_buffer_PLRAM8, s_embedding_buffer_PLRAM9,
        s_embedding_buffer_PLRAM10, s_embedding_buffer_PLRAM11,
        s_embedding_gather_level_A[2]); 
    group_4_embeddings(
        s_embedding_buffer_PLRAM12, s_embedding_buffer_PLRAM13,
        s_embedding_buffer_PLRAM14, s_embedding_buffer_PLRAM15,
        s_embedding_gather_level_A[3]); 
    ...
    group_2_embeddings(
        ...
        s_embedding_gather_level_A[14]); 
    group_2_embeddings(
        s_embedding_buffer_HBM21, s_embedding_buffer_HBM22,
        s_embedding_gather_level_A[15]); 
    group_2_embeddings(
        s_embedding_buffer_HBM23, s_embedding_buffer_HBM24,
        s_embedding_gather_level_A[16]); 
    group_2_embeddings(
        s_embedding_buffer_HBM25, s_embedding_buffer_HBM26,
        s_embedding_gather_level_A[17]); 

    // HBM 27, DDR 0 -> entry len = 16 (4 ap_uint<128>), 2 channels
    group_1_embeddings_16(
        s_embedding_buffer_HBM27,
        s_embedding_gather_level_A[18]);
    group_1_embeddings_16(
        s_embedding_buffer_DDR0,
        s_embedding_gather_level_A[19]);

    // DDR 1 -> entry len = 32 (8 ap_uint<128>), 1 channel
    group_1_embeddings_32(
        s_embedding_buffer_DDR1,
        s_embedding_gather_level_A[20], s_embedding_gather_level_A[21]);

    // gather all 22 streams into 1
    gather_22_embedding_streams(s_embedding_gather_level_A, s_network);
}
