...
{ 
    ...
    float array_buf[BLOCK_ENTRY_NUM_FPGA_SENDER];
    for (int i = 0; i < BLOCK_ENTRY_NUM_FPGA_SENDER; i++) {
        array_buf[i] = 1;
    }
    ...
    for (int i = 0; i < LOOP_NUM * BATCH_NUM_PER_LOOP; i++) {
        ...
        while (total_sent_bytes < BLOCK_SIZE_FPGA_SENDER) {
            int sent_bytes = send(sock, array_buf + total_sent_bytes, BLOCK_SIZE_FPGA_SENDER - total_sent_bytes, 0);
        ...
        if (total_sent_bytes != BLOCK_SIZE_FPGA_SENDER) {
    ...
    float total_size = (float)LOOP_NUM * BATCH_NUM_PER_LOOP * BLOCK_SIZE_FPGA_SENDER;
    printf("Data sent. Packet number:%d\tPacket size:%d bytes\tTotal data:%fGB\n",
        LOOP_NUM * BATCH_NUM_PER_LOOP, BLOCK_SIZE_FPGA_SENDER, total_size / (1024 * 1024 * 1024));   
...
{ 
    ...
    t_info_0.port = PORT_FPGA_SENDER_0 + 0;
    t_info_1.port = PORT_FPGA_SENDER_0 + 1;
    t_info_2.port = PORT_FPGA_SENDER_0 + 2;
    t_info_3.port = PORT_FPGA_SENDER_0 + 3;
