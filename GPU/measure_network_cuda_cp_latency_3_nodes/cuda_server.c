...
timespec network_time_FPGA0[TOTAL_BATCH_NUM]; // when finish receiving a batch from network 
timespec network_time_FPGA1[TOTAL_BATCH_NUM]; // when finish receiving a batch from network 
timespec network_time_CPU0[TOTAL_BATCH_NUM]; // when finish receiving a batch from network 
...
void *thread_receive_packets(void* vargp) 
{
    ...
    int block_entry_num = t_info -> block_entry_num;
    int block_size = t_info -> block_size;
    timespec* network_time = t_info -> time;
    ...
    int* writer_block_id_FPGA0 = t_info -> control_FPGA0 + 0;
    int* writer_iteration_FPGA0 = t_info -> control_FPGA0 + 1;
    int* writer_block_id_FPGA1 = t_info -> control_FPGA1 + 0;
    int* writer_iteration_FPGA1 = t_info -> control_FPGA1 + 1;
    int* writer_block_id_CPU0 = t_info -> control_CPU0 + 0;
    int* writer_iteration_CPU0 = t_info -> control_CPU0 + 1;
   ...
   // Forcefully attaching socket to the port 8080 
    ...
    for (int iter = 0; iter < LOOP_NUM; iter++) {
        ...
        *writer_iteration = iter;
        ...
        for (int block_id = 0; block_id < BATCH_NUM_PER_LOOP; block_id++) {
            ...
            // Wait other receiver threads, ...
            while ((block_id - 1 > *writer_block_id_FPGA0)) {

            }
            while ((block_id - 1 > *writer_block_id_FPGA1)) {
                
            }
            while ((block_id - 1 > *writer_block_id_CPU0)) {
    ...
    printf("Consumed time: %f seconds, INCLUDING Waiting reader proceess\n", elapsed_time);
    printf("Throughput: %f GB / sec", (float)block_size * BATCH_NUM_PER_LOOP * LOOP_NUM / elapsed_time / 1024 / 1024 / 1024);
...
void *thread_consume(void* vargp) {
    ...
    int* writer_block_id_FPGA0 = t_info -> control_FPGA0 + 0;
    int* writer_iteration_FPGA0 = t_info -> control_FPGA0 + 1;
    int* reader_block_id_FPGA0 = t_info -> control_FPGA0 + 2;
    int* reader_iteration_FPGA0 = t_info -> control_FPGA0 + 3;

    int* writer_block_id_FPGA1 = t_info -> control_FPGA1 + 0;
    int* writer_iteration_FPGA1 = t_info -> control_FPGA1 + 1;
    int* reader_block_id_FPGA1 = t_info -> control_FPGA1 + 2;
    int* reader_iteration_FPGA1 = t_info -> control_FPGA1 + 3;
    
    int* writer_block_id_CPU0 = t_info -> control_CPU0 + 0;
    int* writer_iteration_CPU0 = t_info -> control_CPU0 + 1;
    int* reader_block_id_CPU0 = t_info -> control_CPU0 + 2;
    int* reader_iteration_CPU0 = t_info -> control_CPU0 + 3;
    ...
    for (int iter = 0; iter < LOOP_NUM; iter++) {
        ...
        *reader_iteration_FPGA0 = iter;
        *reader_iteration_FPGA1 = iter;
        *reader_iteration_CPU0 = iter;
        // id in the queue
        for (int block_id = 0; block_id < BATCH_NUM_PER_LOOP; block_id++) {
            ...
            //   for 1 blcok
            while ((iter >= *writer_iteration_FPGA0) && (block_id + 1 >= *writer_block_id_FPGA0)) {
                // halt until the reader proceed
                // if last iteration, go through
                if ((iter == LOOP_NUM - 1) 
                    && (*writer_iteration_FPGA0 == LOOP_NUM - 1)
                    && (*writer_block_id_FPGA0 == BATCH_NUM_PER_LOOP - 1)) {
                    break;
                }
            }
            while ((iter >= *writer_iteration_FPGA1) && (block_id + 1 >= *writer_block_id_FPGA1)) {
                // halt until the reader proceed
                // if last iteration, go through
                if ((iter == LOOP_NUM - 1) 
                    && (*writer_iteration_FPGA1 == LOOP_NUM - 1)
                    && (*writer_block_id_FPGA1 == BATCH_NUM_PER_LOOP - 1)) {
                    break;
                }
            }
            while ((iter >= *writer_iteration_CPU0) && (block_id + 1 >= *writer_block_id_CPU0)) {
                // halt until the reader proceed
                // if last iteration, go through
                if ((iter == LOOP_NUM - 1) 
                    && (*writer_iteration_CPU0 == LOOP_NUM - 1)
                    && (*writer_block_id_CPU0 == BATCH_NUM_PER_LOOP - 1)) {
                    break;
                }
            }
            ...
            while ((block_id == BATCH_NUM_PER_LOOP - 1) 
                && (iter == *writer_iteration_FPGA0 - 1) // consider the case when read_iter=-1
                && (*writer_block_id_FPGA0 < 1)) { // consider the case when read_iter=-1
                // halt until the reader proceed
                // if last iteration, go through
                if ((iter == LOOP_NUM - 1) 
                    && (*writer_iteration_FPGA0 == LOOP_NUM - 1)
            ...
            while ((block_id == BATCH_NUM_PER_LOOP - 1) 
                && (iter == *writer_iteration_FPGA1 - 1) // consider the case when read_iter=-1
                && (*writer_block_id_FPGA1 < 1)) { // consider the case when read_iter=-1
                // halt until the reader proceed
                // if last iteration, go through
                if ((iter == LOOP_NUM - 1) 
                    && (*writer_iteration_FPGA1 == LOOP_NUM - 1)
            ...
            while ((block_id == BATCH_NUM_PER_LOOP - 1) 
                && (iter == *writer_iteration_CPU0 - 1) // consider the case when read_iter=-1
                && (*writer_block_id_CPU0 < 1)) { // consider the case when read_iter=-1
                // halt until the reader proceed
                // if last iteration, go through
                if ((iter == LOOP_NUM - 1) 
                    && (*writer_iteration_CPU0 == LOOP_NUM - 1)
            ...
            // write block
            int block_addr_FPGA0 = block_id * BLOCK_ENTRY_NUM_FPGA_SENDER;
            int block_addr_FPGA1 = block_id * BLOCK_ENTRY_NUM_FPGA_SENDER;
            int block_addr_CPU0 = block_id * BLOCK_ENTRY_NUM_CPU_SENDER;

            memcpy(input_feature, t_info -> data_FPGA0 + block_addr_FPGA0, BLOCK_SIZE_FPGA_SENDER);
            memcpy(input_feature + BLOCK_ENTRY_NUM_FPGA_SENDER, t_info -> data_FPGA1 + block_addr_FPGA1, BLOCK_SIZE_FPGA_SENDER);
            memcpy(input_feature + 2 * BLOCK_ENTRY_NUM_FPGA_SENDER, t_info -> data_CPU0 + block_addr_CPU0, BLOCK_SIZE_CPU_SENDER);

            // memcpy(input_feature, t_info -> data_FPGA0 + block_addr_FPGA0, BLOCK_SIZE_FPGA_SENDER);
            // memcpy(input_feature + BLOCK_ENTRY_NUM_FPGA_SENDER, t_info -> data_FPGA1 + block_addr_FPGA1, BLOCK_SIZE_FPGA_SENDER);
            // memcpy(input_feature + 2 * BLOCK_ENTRY_NUM_FPGA_SENDER, t_info -> data_CPU0 + block_addr_CPU0, BLOCK_SIZE_CPU_SENDER);
            *reader_block_id_FPGA0 = block_id;
            *reader_block_id_FPGA1 = block_id;
            *reader_block_id_CPU0 = block_id;
    ...
    checkCudaStatus(cudaMemcpyAsync(r_layer1, d_r_layer1,
        ele_r_layer1 * sizeof(float), cudaMemcpyDeviceToHost, stream));
    checkCudaStatus(cudaStreamSynchronize(stream));
...
int main(int argc, char *argv[]) {
    ...
    //// TODO: more data 
    float* data_FPGA0[THREAD_NUM];
    float* data_FPGA1[THREAD_NUM];
    float* data_CPU0[THREAD_NUM];
    
    for (int i = 0; i < THREAD_NUM; i++) {
        data_FPGA0[i] = malloc(SHM_DATA_SIZE_FPGA); 
        data_FPGA1[i] = malloc(SHM_DATA_SIZE_FPGA); 
        data_CPU0[i] = malloc(SHM_DATA_SIZE_CPU);
        // initialize data block, allocate consecutive memory
        memset(data_FPGA0[i], 0, SHM_DATA_SIZE_FPGA);
        memset(data_FPGA1[i], 0, SHM_DATA_SIZE_FPGA);
        memset(data_CPU0[i], 0, SHM_DATA_SIZE_CPU);
    }
    ...
    int* control_FPGA0[THREAD_NUM];
    int* control_FPGA1[THREAD_NUM];
    int* control_CPU0[THREAD_NUM];
    for (int i = 0; i < THREAD_NUM; i++) {
        control_FPGA0[i] = malloc(4 * sizeof(int)); 
        control_FPGA1[i] = malloc(4 * sizeof(int)); 
        control_CPU0[i] = malloc(4 * sizeof(int)); 
        // initialize control signal
        control_FPGA0[i][0] = -1; // first value: block id of writer (client)
        control_FPGA0[i][1] = -1; // second value: iteration id of writer
        control_FPGA0[i][2] = -1; // third value: block id of reader (CUDA)
        control_FPGA0[i][3] = -1; // fourth value: iteration id of reader
        control_FPGA1[i][0] = -1; // first value: block id of writer (client)
        control_FPGA1[i][1] = -1; // second value: iteration id of writer
        control_FPGA1[i][2] = -1; // third value: block id of reader (CUDA)
        control_FPGA1[i][3] = -1; // fourth value: iteration id of reader
        control_CPU0[i][0] = -1; // first value: block id of writer (client)
        control_CPU0[i][1] = -1; // second value: iteration id of writer
        control_CPU0[i][2] = -1; // third value: block id of reader (CUDA)
        control_CPU0[i][3] = -1; // fourth value: iteration id of reader
    }
    ...
    pthread_t thread_id_store_FPGA0[THREAD_NUM];
    pthread_t thread_id_store_FPGA1[THREAD_NUM];
    pthread_t thread_id_store_CPU0[THREAD_NUM];

    pthread_t thread_id_consume[THREAD_NUM]; 
    printf("Before Thread\n"); 

    struct Receiver_thread_info t_receiver_info_FPGA0[THREAD_NUM];
    struct Receiver_thread_info t_receiver_info_FPGA1[THREAD_NUM];
    struct Receiver_thread_info t_receiver_info_CPU0[THREAD_NUM];
    ...
    for (int i = 0; i < THREAD_NUM; i++) {
        t_receiver_info_FPGA0[i].port = PORT_FPGA_SENDER_0 + i;
        t_receiver_info_FPGA1[i].port = PORT_FPGA_SENDER_1 + i;
        t_receiver_info_CPU0[i].port = PORT_CPU_SENDER_0 + i;

        t_receiver_info_FPGA0[i].data = data_FPGA0[i];
        t_receiver_info_FPGA1[i].data = data_FPGA1[i];
        t_receiver_info_CPU0[i].data = data_CPU0[i];

        t_receiver_info_FPGA0[i].control = control_FPGA0[i];
        t_receiver_info_FPGA1[i].control = control_FPGA1[i];
        t_receiver_info_CPU0[i].control = control_CPU0[i];

        t_receiver_info_FPGA0[i].control_FPGA0 = control_FPGA0[i];
        t_receiver_info_FPGA1[i].control_FPGA0 = control_FPGA0[i];
        t_receiver_info_CPU0[i].control_FPGA0 = control_FPGA0[i];

        t_receiver_info_FPGA0[i].control_FPGA1 = control_FPGA1[i];
        t_receiver_info_FPGA1[i].control_FPGA1 = control_FPGA1[i];
        t_receiver_info_CPU0[i].control_FPGA1 = control_FPGA1[i];

        t_receiver_info_FPGA0[i].control_CPU0 = control_CPU0[i];
        t_receiver_info_FPGA1[i].control_CPU0 = control_CPU0[i];
        t_receiver_info_CPU0[i].control_CPU0 = control_CPU0[i];

        t_receiver_info_FPGA0[i].block_entry_num = BLOCK_ENTRY_NUM_FPGA_SENDER;
        t_receiver_info_FPGA1[i].block_entry_num = BLOCK_ENTRY_NUM_FPGA_SENDER;
        t_receiver_info_CPU0[i].block_entry_num = BLOCK_ENTRY_NUM_CPU_SENDER;

        t_receiver_info_FPGA0[i].block_size = BLOCK_SIZE_FPGA_SENDER;
        t_receiver_info_FPGA1[i].block_size = BLOCK_SIZE_FPGA_SENDER;
        t_receiver_info_CPU0[i].block_size = BLOCK_SIZE_CPU_SENDER;

        t_receiver_info_FPGA0[i].time = network_time_FPGA0;
        t_receiver_info_FPGA1[i].time = network_time_FPGA1;
        t_receiver_info_CPU0[i].time = network_time_CPU0;
    }
    ...
    // TODO
    for (int i = 0; i < THREAD_NUM; i++) {
        t_cuda_info[i].data_FPGA0 = data_FPGA0[i];
        t_cuda_info[i].data_FPGA1 = data_FPGA1[i];
        t_cuda_info[i].data_CPU0 = data_CPU0[i];

        t_cuda_info[i].control_FPGA0 = control_FPGA0[i];
        t_cuda_info[i].control_FPGA1 = control_FPGA1[i];
        t_cuda_info[i].control_CPU0 = control_CPU0[i];
    ...
    // pthread_t thread_id_store_CPU0[THREAD_NUM];
    for (int i = 0; i < THREAD_NUM; i++) {
        pthread_create(&thread_id_store_FPGA0[i], NULL, thread_receive_packets, (void*) &t_receiver_info_FPGA0[i]);
        pthread_create(&thread_id_store_FPGA1[i], NULL, thread_receive_packets, (void*) &t_receiver_info_FPGA1[i]);
        pthread_create(&thread_id_store_CPU0[i], NULL, thread_receive_packets, (void*) &t_receiver_info_CPU0[i]);
    }
    ...
    for (int i = 0; i < THREAD_NUM; i++) {
        pthread_join(thread_id_store_FPGA0[i], NULL); 
        pthread_join(thread_id_store_FPGA1[i], NULL); 
        pthread_join(thread_id_store_CPU0[i], NULL); 
    }
    ...
    timespec memcpy_timespec_FPGA0[TOTAL_BATCH_NUM];
    timespec memcpy_timespec_FPGA1[TOTAL_BATCH_NUM];
    timespec memcpy_timespec_CPU0[TOTAL_BATCH_NUM];
    long memcpy_time_ns_FPGA0[TOTAL_BATCH_NUM];
    long memcpy_time_ns_FPGA1[TOTAL_BATCH_NUM];
    long memcpy_time_ns_CPU0[TOTAL_BATCH_NUM];
    ...
    for (int i = 0; i < TOTAL_BATCH_NUM; i++) {
        memcpy_timespec_FPGA0[i] = diff(network_time_FPGA0[i], cuda_time[i]);
        memcpy_timespec_FPGA1[i] = diff(network_time_FPGA1[i], cuda_time[i]);
        memcpy_timespec_CPU0[i] = diff(network_time_CPU0[i], cuda_time[i]);

        memcpy_time_ns_FPGA0[i] = memcpy_timespec_FPGA0[i].tv_sec * 1000 * 1000 * 1000 + memcpy_timespec_FPGA0[i].tv_nsec;
        memcpy_time_ns_FPGA1[i] = memcpy_timespec_FPGA1[i].tv_sec * 1000 * 1000 * 1000 + memcpy_timespec_FPGA1[i].tv_nsec;
        memcpy_time_ns_CPU0[i] = memcpy_timespec_CPU0[i].tv_sec * 1000 * 1000 * 1000 + memcpy_timespec_CPU0[i].tv_nsec;

        long largest = (memcpy_time_ns_FPGA0[i] + memcpy_time_ns_FPGA1[i]) / 2;
        // long largest =  memcpy_time_ns_FPGA0[i] > memcpy_time_ns_FPGA1[i]? memcpy_time_ns_FPGA0[i] : memcpy_time_ns_FPGA1[i];
        // largest = largest > memcpy_time_ns_CPU0[i]? largest : memcpy_time_ns_CPU0[i];
        // long largest = (memcpy_time_ns_FPGA0[i] + memcpy_time_ns_FPGA1[i] + memcpy_time_ns_CPU0[i]) / 3;
        // long largest =  memcpy_time_ns_FPGA0[i] < memcpy_time_ns_FPGA1[i]? memcpy_time_ns_FPGA0[i] : memcpy_time_ns_FPGA1[i];
        // largest = largest < memcpy_time_ns_CPU0[i]? largest : memcpy_time_ns_CPU0[i];


        memcpy_time_ns[i] = largest;

        total_memcpy_ns_fl += (float) memcpy_time_ns[i];
        printf("i = %d FPGA0 time = %ld ns\n", i, memcpy_time_ns_FPGA0[i]);
        printf("i = %d FPGA1 time = %ld ns\n", i, memcpy_time_ns_FPGA1[i]);
        printf("i = %d CPU0 time = %ld ns\n", i, memcpy_time_ns_CPU0[i]);
