...
{ 
    ...
    float array_buf[BLOCK_ENTRY_NUM];
    ...
    for (int i = 0; i < BATCH_NUM_PER_THREAD; i++) {
    ...
    float total_size = (float)BATCH_NUM_PER_THREAD * BLOCK_SIZE;
    printf("Data sent. Packet number:%d\tPacket size:%d bytes\tTotal data:%fGB\n",
        BATCH_NUM_PER_THREAD, BLOCK_SIZE, total_size / (1024 * 1024 * 1024));   
