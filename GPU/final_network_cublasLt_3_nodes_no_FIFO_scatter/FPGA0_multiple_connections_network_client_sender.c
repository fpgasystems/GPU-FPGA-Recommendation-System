...
#define DEBUG
    ...
    // float total_size = (float)BATCH_NUM_PER_THREAD * BLOCK_SIZE_FPGA_SENDER;
    // printf("Data sent. Packet number:%d\tPacket size:%d bytes\tTotal data:%fGB\n",
    //     BATCH_NUM_PER_THREAD, BLOCK_SIZE_FPGA_SENDER, total_size / (1024 * 1024 * 1024));   
...
{ 
    ...
    for (int i = 0; i < THREAD_NUM; i++) {
        t_info[i].port = PORT_FPGA_SENDER_0 + i;
    }
